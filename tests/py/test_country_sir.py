...
def solve_sir(beta, gamma, y0, t_eval, *, N):
    ...
    results = solve_ivp(rhs, y0=(*y0, 0, 0, 0, 0, 0, 0),
                        t_span=(0, t_eval[-1]), t_eval=t_eval, rtol=1e-8, atol=1e-8)
...
class TestCountrySIR(TestCaseEx):
    def test_sir(self):
        ...
        for py, noad, ad in zip(py_result[1:], cpp_result_noad[1:], cpp_result_ad[1:]):
            ...
            self.assertRelative(py[3], ad.S().d(0),  tolerance=1e-6)
            self.assertRelative(py[4], ad.I().d(0),  tolerance=1e-6)
            self.assertRelative(py[5], ad.R().d(0),  tolerance=1e-6)
            self.assertRelative(py[6], ad.S().d(1),  tolerance=1e-6)
            self.assertRelative(py[7], ad.I().d(1),  tolerance=1e-6)
            self.assertRelative(py[8], ad.R().d(1),  tolerance=1e-6)
