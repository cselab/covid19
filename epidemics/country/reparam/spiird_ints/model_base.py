...
class ModelBase( EpidemicsCountry ):
  ...
  def solve_ode( self, y0, T, t_eval, N, p ):
    ...
    spiird_ints = libepidemics.country.spiird_ints_reparam
    dp          = libepidemics.country.DesignParameters(N=N)
    cppsolver   = spiird_ints.Solver(dp)

    params = spiird_ints.Parameters(R0=p[0], D=p[1], Y=p[2], alpha=p[3], eps=p[4], tact=self.intday+p[5], kbeta=p[6])
    ...
    initial = spiird_ints.State(y0cpp)
