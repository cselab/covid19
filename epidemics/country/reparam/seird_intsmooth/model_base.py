...
class ModelBase( EpidemicsCountry ):
  ...
  def solve_ode( self, y0, T, t_eval, N, p ):
    ...
    seird_intsmooth_reparam  = libepidemics.country.seird_intsmooth
    dp        = libepidemics.country.DesignParameters(N=N)
    cppsolver = seird_intsmooth_reparam.Solver(dp)

    params = seird_intsmooth_reparam.Parameters(R0=p[0], D=p[1], Z=p[2], eps=p[3], tact=p[4], dtact=p[5], kbeta=p[6])
    
    s0, i0  = y0
    y0cpp   = (s0, p[0]*i0, i0, 0.0, 0.0) # S E I R D
    initial = seird_intmooth_reparam.State(y0cpp)
