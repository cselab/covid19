...
def cut_data_intervention(cases,region):
    print('[Epidemics][Preprocessing]')
    print('     Removing data pre intervention')

    start_date = cases.start_date

    lockdown_date = get_lockdown_date(region)

    n_days = (lockdown_date-start_date).days + 14

    print('     Using {} first days'.format(n_days))
...
def get_lockdown_date(region, path= "official_lockdowns.csv"):
    """
    ...
    """
    csv = pd.read_csv(DATA_FILES_DIR / path)
    ...
    idx = df.index[df['country'] == region.capitalize()].tolist()
    date = df.iloc[idx]['date'].iloc[0]

    print('     Intevention data for {}: {}'.format(region,date))
