...
def plot_comparison_single_model(data_path,variable,save_dir,regions):
    ...
    regions_folders = [data_path+region.replace(' ', '')+'/'+file_path for region in regions]
...
if __name__ == "__main__":  
    ...
    parser.add_argument('--data_path', '-p', default='1', help='Which phases to plor.')
    parser.add_argument('--variable', '-v', default='R0', help='Model type')
    parser.add_argument('--save_dir', '-sd', default='data/_figures/', help='Model type')

    args = parser.parse_args()

    regions = EUROPEAN_COUNTRIES
    plot_comparison_single_model(args.data_path,args.variable,args.save_dir,regions)
